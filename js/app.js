...
const BUILD_INFO = Object.freeze({
  version: '1.0.0-rc1.1',
  channel: 'release-candidate-hotfix',
  codeFreeze: true
});

let runtimeRecoveryShown = false;

function showRuntimeRecoveryInline(code = 'UNKNOWN', detail = '') {
  if (runtimeRecoveryShown) return;
  runtimeRecoveryShown = true;

  const recovery = document.getElementById('runtimeRecovery');
  const codeElement = document.getElementById('runtimeErrorCode');

  if (!recovery) return;

  if (codeElement) {
    codeElement.textContent = detail
      ? `${code}: ${String(detail).slice(0, 120)}`
      : code;
  }
...
function hideRuntimeRecoveryInline() {
  runtimeRecoveryShown = false;
  document.getElementById('runtimeRecovery')?.classList.add('hidden');
}

window.addEventListener('error', event => {
  showRuntimeRecoveryInline(
    'JS_ERROR',
...
document.getElementById('runtimeRetryBtn')?.addEventListener('click', () => {
  hideRuntimeRecoveryInline();
  window.location.reload();
});

document.getElementById('runtimeReloadBtn')?.addEventListener('click', async () => {
  try {
    const registrations =
    ...
    console.warn('Full cache reset unavailable:', error);
  }
...
window.setTimeout(() => {
  ...
  if (loadingView && !loadingView.classList.contains('hidden')) {
    showRuntimeRecoveryInline('BOOT_TIMEOUT');
  }
}, RC_BOOT_TIMEOUT_MS);
